...
function (angular, app, $, _, kbn, moment, timeSeries) {
  ...
  var module = angular.module('kibana.panels.filteredcount', []);
  app.useModule(module);

  module.controller('filteredcount', function($scope, $q, querySrv, dashboard, filterSrv, fields) {
    $scope.panelMeta = {
      modals : [
        {
          description: "Inspect",
    ...
    $scope.burtFilter = false;
     // filterSrv.removeByTypeAndField('terms',$scope.panel.filterField);
   // $scope.panel.filterVal = '';
    //Set Burt Filter
    $scope.setBurtFilter = function() {
        if($scope.panel.filterField !== '' && $scope.panel.filterVal !== '') {
            filterSrv.set({type:'terms',field:$scope.panel.filterField,value:$scope.panel.filterVal, mandate: 'must',active:true});
        }
        else {
            filterSrv.removeByTypeAndField('terms',$scope.panel.filterField);
        }
  ...
  module.directive('burtcountChart', function(dashboard, filterSrv) {
    return {
      restrict: 'A',
      template: '<div></div>',
        ...
        elem.bind("plothover", function (event, pos, item) {
          ...
          if (item) {
            ...
            if (scope.panel.stack && scope.panel.tooltip.value_type === 'individual')  {
              value = item.datapoint[1] - (item.datapoint[2] || 0);
            } else {
              value = item.datapoint[1];
            }
            $tooltip
              .html(
                group + value + " @ " + moment.utc(item.datapoint[0]).format('MM/DD HH:mm:ss')
              )
